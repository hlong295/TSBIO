    ...
    <div className="min-h-screen bg-white pb-20">
      ...
      <main className="max-w-screen-sm mx-auto">
        ...
        <div className="sticky top-14 z-40 bg-white border-b px-4 py-2">
          <Link href="/nong-san">
            <Button variant="ghost" size="sm" className="h-8 px-2 text-xs hover:bg-transparent bg-transparent">
        ...
        <section className="px-4 py-4 space-y-4">
          ...
          <div className="space-y-2">
            <h1 className="text-base font-bold text-foreground leading-tight">{product.name}</h1>
          ...
          <div className="space-y-1">
            ...
            {product.piPrice && (
              <p className="text-xs text-accent font-medium">Hoặc {product.piPrice}</p>
            )}
          </div>
          ...
          <div className="space-y-1.5 py-3 border-y">
            <div className="flex items-start gap-2">
              ...
              <div className="text-xs">
                <p className="font-medium text-foreground">{product.farm}</p>
            ...
            <Card className="bg-amber-50 border-amber-200">
              <CardContent className="p-3">
                <div className="flex items-center gap-2">
                  <Wallet className={`h-4 w-4 ${isWalletConnected ? "text-amber-600" : "text-muted-foreground"}`} />
              ...
              {expandedVerified && (
                <div className="mt-3 space-y-2 pt-3 border-t border-primary/10">
                  ...
                  <div className="space-y-1.5">
                    <p className="text-xs font-medium text-foreground">Chứng nhận:</p>
                    <div className="flex flex-wrap gap-1.5">
                      {product.certifications.map((cert, index) => (
                        <Badge key={index} variant="outline" className="text-[10px] h-5 bg-white">
                          {cert}
                        </Badge>
                      ))}
                    </div>
                ...
                <Link key={related.id} href={`/nong-san/chi-tiet?id=${related.id}`}>
                  <Card className="bg-white border-border overflow-hidden hover:border-primary/50 transition-colors cursor-pointer">
                    ...
                    <CardContent className="p-2.5">
                      <h3 className="font-semibold text-xs leading-tight line-clamp-1 mb-1">{related.name}</h3>
