...
// Account page depends on client-side auth state (Pi SDK / local session).
// Force dynamic rendering to avoid build-time prerender crashes on Vercel.
...
export default function AccountPage() {
  ...
  const { isAuthenticated, piUser, logout, updateDisplayName } = useAuth()
  const [isEditingName, setIsEditingName] = useState(false)
  const [newDisplayName, setNewDisplayName] = useState(piUser?.displayName || piUser?.username || "")

  // Keep display name in sync if user changes (e.g. after login refresh)
  useEffect(() => {
    setNewDisplayName(piUser?.displayName || piUser?.username || "")
  }, [piUser?.displayName, piUser?.username])
  ...
  // IMPORTANT: During Next.js build/prerender, auth state can be null.
  // Keep render safe even when user is not yet available.
  if (!piUser) {
    return (
  ...
  const displayName = piUser.displayName || piUser.username
  const isAdmin = piUser.role === "admin"
  const createdDate = piUser.createdAt
    ? new Date(piUser.createdAt).toLocaleDateString("vi-VN", {
        year: "numeric",
        month: "long",
        day: "numeric",
      })
