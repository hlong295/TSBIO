...
export async function POST(req: Request, ctx: { params: { id: string } }) {
  ...
  async function resolveToFinal(item: IncomingMedia) {
    if (item.tmp_path) {
      if (!item.tmp_path.startsWith("tmp/")) throw new Error("INVALID_TMP_PATH");
      ...
      return { path: finalPath, url: pub.publicUrl };
    }
  ...
  try {
    ...
    const resolvedNewVideo = incomingVideos.length ? await resolveToFinal(incomingVideos[0]) : null;
    ...
    const mergedVideo: StoredMediaItem | null = resolvedNewVideo
      ? { url: resolvedNewVideo.url, type: "video" as const, path: resolvedNewVideo.path, display_order: 0 }
      : existingVideo;
