...
export async function POST(req: Request, ctx: { params: { id: string } }) {
  ...
  try {
    ...
    const normalized: AttachItem[] = items
      .map((it: any) => {
        const raw = it?.path;
        const path = (typeof raw === "string" ? raw : String(raw ?? "")).trim();
        const type = it?.type;
        return { path, type } as AttachItem;
      })
      .filter((it) => it.path.length > 0);

    const dedup: AttachItem[] = Array.from(new Map(normalized.map((x) => [x.path, x])).values());
