...
export async function POST(req: Request) {
  ...
  const name = (typeof body?.name==="string"?body?.name.trim():String(body?.name??""));
  const inputSlug = (typeof body?.slug==="string"?body?.slug.trim():String(body?.slug??""));
  const hubId = (typeof body?.hub_id==="string"?body?.hub_id.trim():String(body?.hub_id??"")) || null;
  const code = (typeof body?.code==="string"?body?.code.trim():String(body?.code??"")) || null;
