...
export async function PUT(req: Request) {
  try {
    ...
    // Admin UI always submits the full payload, so we write all banner keys.
    // This avoids edge cases where a nested payload shape causes some fields
    // to be skipped and makes Home never reflect the saved config.
    const toWrite: Array<[string, string]> = Object.entries(next);
