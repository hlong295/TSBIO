          ...
          <>
            ...
            <div className="grid gap-2">
              ...
              <AdminRichTextEditor
                ...
                onChange={(html) => setItem({ ...item, description: html })}
                placeholder="Mô tả (dán từ Word/Docs được)"
              />
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              <div className="flex items-center gap-3">
                <Switch checked={!!item.is_combo} onCheckedChange={(v) => setItem({ ...item, is_combo: v })} />
                <div className="text-sm">Combo</div>
              </div>
              <div className="flex items-center gap-3">
                <Switch checked={!!item.is_featured} onCheckedChange={(v) => setItem({ ...item, is_featured: v })} />
                <div className="text-sm">Featured</div>
              </div>
              <div className="flex items-center gap-3">
                <Switch checked={!!item.is_flashsale} onCheckedChange={(v) => setItem({ ...item, is_flashsale: v })} />
                <div className="text-sm">Flashsale</div>
              </div>
              <div className="flex items-center gap-3">
                <Switch checked={!!item.is_verified} onCheckedChange={(v) => setItem({ ...item, is_verified: v })} />
                <div className="text-sm">Verified</div>
              </div>
              <div className="flex items-center gap-3">
                <Switch checked={!!item.is_archived} onCheckedChange={(v) => setItem({ ...item, is_archived: v })} />
            ...
            {item.is_flashsale ? (
              <div className="grid gap-2">
                <div className="text-sm font-medium">Flashsale config (optional)</div>
                <Input
                  value={item.flashsale_percent ?? ""}
                  onChange={(e) => setItem({ ...item, flashsale_percent: e.target.value })}
                  placeholder="flashsale_percent (e.g. 20)"
                />
                <Input
                  type="datetime-local"
                  value={item.flashsale_end_at ? new Date(item.flashsale_end_at).toISOString().slice(0, 16) : ""}
                  onChange={(e) => setItem({ ...item, flashsale_end_at: e.target.value ? new Date(e.target.value).toISOString() : null })}
                />
              </div>
            ) : null}

            <AdminProductMedia productId={params.id} />
              ...
              <Button
                ...
                onClick={() =>
                  patchAndSave({
                    ...
                    flashsale_percent:
                      item.flashsale_percent === ""
                        ? null
                        : item.flashsale_percent === undefined
                          ? undefined
                          : Number(item.flashsale_percent),
                    flashsale_end_at: item.flashsale_end_at || null,
