          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                // If media is selected during creation, upload/attach right after create.
                if (id) {
                  try {
                    const mediaHeaders = { ...(await getAdminBearer()) } as any;

                    if (mediaDraft.deviceImages?.length) {
                      setStatus(`Đang upload ${mediaDraft.deviceImages.length} ảnh...`);
                      const form = new FormData();
                      for (const f of mediaDraft.deviceImages) form.append("files", f);
                      const r = await fetch(`/api/admin/products/${id}/media?kind=image`, {
                        method: "POST",
                        headers: mediaHeaders,
                        body: form,
                      });
                      const j = await r.json().catch(() => ({}));
                      if (!r.ok) throw new Error(`${j?.error || "UPLOAD_IMAGES_FAILED"}${j?.detail ? `: ${j.detail}` : ""}`);
                    }

                    if (mediaDraft.deviceVideo) {
                      setStatus("Đang upload video...");
                      const form = new FormData();
                      form.append("file", mediaDraft.deviceVideo);
                      const r = await fetch(`/api/admin/products/${id}/media?kind=video`, {
                        method: "POST",
                        headers: mediaHeaders,
                        body: form,
                      });
                      const j = await r.json().catch(() => ({}));
                      if (!r.ok) throw new Error(`${j?.error || "UPLOAD_VIDEO_FAILED"}${j?.detail ? `: ${j.detail}` : ""}`);
                    }

                    if (mediaDraft.library?.length) {
                      setStatus(`Đang attach ${mediaDraft.library.length} media từ thư viện...`);
                      const r = await fetch(`/api/admin/products/${id}/media/attach`, {
                        method: "POST",
                        headers: { "Content-Type": "application/json", ...(await getAdminBearer()) } as any,
                        body: JSON.stringify({ items: mediaDraft.library }),
                      });
                      const j = await r.json().catch(() => ({}));
                      if (!r.ok) throw new Error(`${j?.error || "ATTACH_MEDIA_FAILED"}${j?.detail ? `: ${j.detail}` : ""}`);
                    }
                  } catch (e: any) {
