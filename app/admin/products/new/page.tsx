...
export default function AdminProductNewPage() {
  ...
  // Cache admin bearer headers for reuse (media upload + API calls).
...
useEffect(() => {
  if (!token) return;
  setAdminHeaders({ Authorization: `Bearer ${token}` });
  (async () => {
    try {
  ...
  return (
    <AdminShell title="New Product">
      <div className="flex flex-col gap-4 max-w-xl">
        ...
        <AdminProductMediaDraft
          value={mediaDraft}
          onChange={setMediaDraft}
          disabled={saving}
          apiHeaders={adminHeaders}
        />
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                if (id) {
                  const hasPending =
                    (mediaDraft.images || []).some((x) => x.status === "uploading") ||
                    (mediaDraft.video ? mediaDraft.video.status === "uploading" : false);
                  if (hasPending) {
                    ...
                    try {
                      ...
                      for (const url of mediaDraft.libraryImages || []) {
                        if (url) items.push({ kind: "image", url, name: url });
                      }
                      if (mediaDraft.libraryVideoUrl) {
                        items.push({ kind: "video", url: mediaDraft.libraryVideoUrl, name: mediaDraft.libraryVideoUrl });
                      }
