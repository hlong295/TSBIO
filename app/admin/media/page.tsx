...
export default function AdminMediaPage() {
  ...
  const upload = async () => {
    ...
    setError(null);
    try {
      // Use the same proven flow as product media draft: server Route Handler uploads via admin client.
      const bearer = (await getAdminBearer()) as any;
      if (!bearer?.Authorization) throw new Error("NO_SESSION");

      const url = new URL("/api/admin/media/tmp-upload", window.location.origin);
      url.searchParams.set("prefix", prefix || "uploads/");
