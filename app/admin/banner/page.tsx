...
async function getBearer() {
  ...
  // Fallback (legacy): in case some environments cached it.
  try {
...
export default function AdminBannerPage() {
  ...
  async function onPickFile(file?: File | null) {
    ...
    try {
      ...
      const r = await fetch("/api/admin/media/upload?folder=banners", {
        method: "POST",
        headers: {
          ...(bearer ? { Authorization: `Bearer ${bearer}` } : {}),
        },
      ...
      setData((prev) => ({ ...prev, heroImageUrl: j.publicUrl }));
        ...
        <div className="rounded-xl border bg-white p-4 shadow-sm">
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              />
              <Input
                value={data.ctaSupportHref}
                onChange={(e) => setData((p) => ({ ...p, ctaSupportHref: e.target.value }))}
                placeholder="Link CTA (vd: /cuu-vuon)"
              />
            </div>
          </div>
