...
export default function TinTucPage() {
  ...
  const ArticleCard = ({ article, featured = false }: { article: NewsArticle, featured?: boolean }) => {
    ...
    return (
      <Card className="bg-white border-border overflow-hidden cursor-pointer hover:border-primary/50 transition-colors">
        <CardContent className={featured ? "p-3" : "p-3"}>
          <div className="flex gap-3">
            ...
            <div className="flex-1 space-y-1.5">
              <Badge className={`text-[9px] h-4 px-1.5 ${categoryColors[article.category]}`}>
                <CategoryIcon className="h-2 w-2 mr-0.5" />
                {categoryLabels[article.category]}
              </Badge>

              <h3 className={`font-semibold leading-tight ${featured ? 'text-sm' : 'text-xs'} line-clamp-2`}>
                {article.title}
              </h3>
      ...
      <main className="px-4 py-4 space-y-5 max-w-screen-sm mx-auto">
        ...
        <Tabs defaultValue="all" className="w-full">
          <TabsList className="grid w-full grid-cols-4 h-9 bg-secondary">
            ...
            <TabsTrigger value="knowledge" className="text-xs data-[state=active]:bg-white">
              Kiến thức
            </TabsTrigger>
            <TabsTrigger value="experience" className="text-xs data-[state=active]:bg-white">
              Kinh nghiệm
            </TabsTrigger>
            <TabsTrigger value="market" className="text-xs data-[state=active]:bg-white">
              Thị trường
            </TabsTrigger>
          </TabsList>

          <TabsContent value="all" className="space-y-2.5 mt-3">
            {regularArticles.map((article) => (
              <ArticleCard key={article.id} article={article} />
            ))}
          </TabsContent>

          <TabsContent value="knowledge" className="space-y-2.5 mt-3">
            {newsArticles.filter(a => a.category === "knowledge").map((article) => (
              <ArticleCard key={article.id} article={article} />
            ))}
          </TabsContent>

          <TabsContent value="experience" className="space-y-2.5 mt-3">
            {newsArticles.filter(a => a.category === "experience").map((article) => (
              <ArticleCard key={article.id} article={article} />
            ))}
          </TabsContent>

          <TabsContent value="market" className="space-y-2.5 mt-3">
            {newsArticles.filter(a => a.category === "market").map((article) => (
              <ArticleCard key={article.id} article={article} />
            ))}
          </TabsContent>
        </Tabs>
