...
export default function TSBIOApp() {
  const [hero, setHero] = React.useState<HomeHeroState>(DEFAULT_HERO)
  ...
  React.useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!next) return
        if (!isMounted) return
        const headlineTop = String(next.headlineTop || "")
        const headlineMid = String(next.headlineMid || "")
        const headlineBottom = String(next.headlineBottom || "")
        setHero({
          headlineTop: headlineTop || DEFAULT_HERO.headlineTop,
          headlineMid: headlineMid || DEFAULT_HERO.headlineMid,
          headlineBottom: headlineBottom || DEFAULT_HERO.headlineBottom,
          heroImageUrl: String(next.heroImageUrl || "") || DEFAULT_HERO.heroImageUrl,
	          ctaSupportLabel: String(next.ctaSupportLabel || "") || DEFAULT_HERO.ctaSupportLabel,
	          ctaSupportHref: String(next.ctaSupportHref || "") || DEFAULT_HERO.ctaSupportHref,
          searchPlaceholder: String(next.searchPlaceholder || "") || DEFAULT_HERO.searchPlaceholder,
          diagnoseButtonLabel: String(next.diagnoseButtonLabel || "") || DEFAULT_HERO.diagnoseButtonLabel,
        })
      } catch {
        // Keep defaults
      }
  ...
  return (
    <div className="min-h-screen bg-white pb-24 w-full max-w-full overflow-x-hidden">
      ...
      {/* Main Content */}
      <main className="w-full max-w-[430px] mx-auto pt-header">
        {/* Hero Section with Overlay Blocks */}
        <section className="relative w-full max-w-full">
          {/* Hero Banner with Rich Harvest Background */}
          <div className="overflow-hidden relative min-h-[300px] sm:min-h-[340px] flex flex-col items-center justify-center w-full py-6">
            {/* Rich Harvest Background Image - Full clarity, no overlays */}
            <Image
              src={hero.heroImageUrl || "/tsbio-harvest-hero.jpg"}
            ...
            <div className="relative z-10 text-center text-white px-6">
              <h2 
                className="text-xl font-bold leading-tight mb-2 text-balance whitespace-pre-line" 
                style={{ 
              ...
              <div className="mt-3 flex justify-center">
                ...
                >
                  <a href={hero.ctaSupportHref || "/cuu-vuon"}>
                    {hero.ctaSupportLabel || "HỖ TRỢ KỸ THUẬT CHUYÊN SÂU"}
                  </a>
                </Button>
              </div>
              ...
              <div className="mt-3">
                <HeroSearch
                  ...
                  buttonLabel={hero.diagnoseButtonLabel}
                />
              </div>
            </div>
          </div>
          ...
          {/* FIX (A0 pixel-lock): prevent tiles from overlapping hero content on small/iOS viewports */}
          <div className="relative z-20 px-4 mt-4 pb-6">
