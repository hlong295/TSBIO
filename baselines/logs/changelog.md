...
* C3 nâng cao — Realtime upload + progress (retry-safe)
  * Add temp upload endpoint: `/api/admin/media/tmp-upload` (POST upload to Storage `media` under `tmp/{userId}/...`; DELETE remove tmp)
  * Add commit endpoint: `/api/admin/products/[id]/media/commit` (move tmp objects to `products/{id}/images|video`, merge with library URLs, update `products.media`, `image_url`, `video_url`)
  * Update Admin Product "New" flow: upload happens immediately; Save only binds committed media; blocks Save while uploads pending
  * Update UI logic (no layout change): show per-file progress text; enforce 10 images + 1 video on client + server
...
## 2026-02-08 | v27_fix20

* Phase B1 - Product Management upgrade (flags + WYSIWYG)
  * Admin product new/edit: replace Description textarea with Tiptap WYSIWYG editor (paste Word/Docs ok)
  * Server-side sanitize for product.description on create/update
  * Add product flags in admin UI + API payloads: is_combo, is_featured, is_flashsale (+ percent/end_at), is_verified, is_archived
  * New DB safe add script: baselines/database/TSBIO_DB_20260208_v27_phaseB1_products_flags_wysiwyg.sql


## 2026-02-09 | v28_fix21

* Phase B1 - C3 Media (product media: 1 video + 10 images)
  * Add admin product media UI in Edit page (upload/remove, show thumbs)
  * Server API guards enforce limits (max 10 images + 1 video)
  * Upload path convention inside bucket `media`:
    * `products/{productId}/images/*`
    * `products/{productId}/video/*`
  * Persist metadata into `products.media` (jsonb) and derived columns `thumbnail_url`, `video_url`
  * Audit action: `product.media.update`
  * New DB safe add script: baselines/database/TSBIO_DB_20260209_v28_phaseB1_product_media.sql


## 2026-02-09 | v29_fix22

* Phase B1 - C3 Media UX (Create product can pick media immediately)
  * Admin product New page now supports selecting media **before Save**:
    * Option A: Upload from device (images/video)
    * Option B: Pick from existing Admin Media library (copies into product folder)
  * On Save: create product -> upload device files -> attach library picks -> redirect to Edit
  * New server API: `/api/admin/products/[id]/media/attach` (copy from media library, enforces limits, audit `product.media.update`)
  * Security: fix `/api/admin/media/list` to properly enforce role guard response


## 2026-02-09 | v34_fix27

* Phase B1 - Media foundation (Supabase Storage)
  * Add server helper `ensureStorageBucket()` (best-effort)
  * Auto-ensure Storage bucket `media` exists and is public before:
    * `/api/admin/media/sign-upload`
    * `/api/admin/media/upload`
    * `/api/admin/media/list`
    * `/api/admin/products/[id]/media`
  * Add DB script to create bucket + public-read policy: `baselines/database/TSBIO_DB_20260209_v34_media_storage_bucket.sql`
