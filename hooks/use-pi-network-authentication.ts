...
export function usePiNetworkAuthentication() {
  ...
  // Keep scopes minimal for login to avoid "silent hang" when Payments permission isn't configured.
  // Request "payments" later only when initiating a payment.
  const loginScopes = useMemo(() => ["username"], [])
  ...
  const initSdk = useCallback(async () => {
    ...
    try {
      ...
      // Practical guard: Pi Browser only injects a fully working SDK when the app is opened
      // on a domain that is configured/approved in Pi Developer Portal.
      // With ephemeral domains like *.vercel.app, Pi login commonly hangs forever.
      if (
        isPiBrowser() &&
        ...
        throw new Error(
          "PI_DOMAIN_NOT_ALLOWED: Pi SDK thường KHÔNG hoạt động trên *.vercel.app. Hãy mở app bằng domain đã khai báo (vd tsbio.vn) hoặc URL Pi App Studio (*.pinet.com)."
        )
      }
