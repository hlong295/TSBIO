...
- Working branch output: **v30_fix23 (Save + redirect after create)**.
 - Working branch output (this change): **v32_fix25 (Admin session UI no longer blocks on token hydration; cached token fallback; avoids stuck "Đang kiểm tra phiên Admin..." when switching tabs)**.

- Working branch output (this change): **v34_fix27 (Media foundation: auto-ensure Supabase Storage bucket `media` exists/public before Media/Product upload/list/sign; adds DB script for bucket/policies)**.

- Working branch output (this change): **v35_fix28 (Realtime media upload + progress + retry-safe commit: upload to temp storage immediately, show % progress; on Save only binds media by committing temp paths into /products/{id}/..., updates products.media + image_url/video_url; no UI layout changes)**.

- Working branch output (this change): **v42_fix35 (Fix runtime crash on /admin/products/new: `adminBearerHeaders is not defined` → cache headers via `getAdminBearer()` and pass to media draft + API calls; no UI/layout changes)**.

- Working branch output (this change): **v50_fix?? (Anti-prerender crash: convert `/cap-nhat-mat-khau` to server wrapper + client component; remove segment config exports from client module to satisfy Next export requirements; no UI change)**.
...
- Media foundation: ensure Storage bucket `media` exists (public) + best-effort auto-create via server routes; add DB snapshot/migration script (v34_fix27).
- C3 nâng cao: realtime upload ngay khi chọn file + hiển thị progress %, lưu tạm storage tmp/, Save chỉ commit/bind media (retry-safe) (v35_fix28).
- Fix: /admin/products/new không còn crash client với lỗi `adminBearerHeaders is not defined`; media draft nhận đúng headers (v42_fix35).
- Fix: Vercel build không còn fail prerender `/cap-nhat-mat-khau` do `revalidate` export trong client page; page chuyển sang server wrapper + client component (v50).

## Now:
- Verify on Vercel/Pi App Studio:
  - login `hlong295` → /admin → wait 2–5 min → click tabs; should NOT get stuck on “Đang kiểm tra phiên Admin...” and should keep Admin usable.
  - open /admin/media and /admin/products/new then upload media: should not fail due to missing bucket.
  - /admin/products/new: chọn ảnh/video → upload ngay và thấy %; bấm Lưu → không re-upload file, chỉ commit/bind; refresh/retry không tạo media rác.
...
- Supabase Storage bucket `media` exists (DB script provided + server auto-ensure added). If Storage policies are customized, confirm desired public/private model.
...
- DB migration: `baselines/database/TSBIO_DB_20260209_v34_media_storage_bucket.sql`