...
- FIX (v65): Product form media picker uses explicit button→inputRef click (avoid display:none) so file dialog + onChange works reliably; "Chọn từ thư viện" enabled (opens /admin/media).

## Now:
- Verify B1 on user env: upload images/videos on `/admin/products/new` shows pending/progress rows + previews; can proceed to Save and commit to product.
