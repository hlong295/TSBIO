...
## State:
- Current baseline source: **TSBIO_SRC_20260208_v27_A1_bannerAdmin_fix16.zip**
...
## Update (2026-02-08):
- Fix A1 Hero Banner config not reflecting on Home:
  - /api/admin/banner now accepts Admin UI payload shape `{ settings: {...} }` (and direct payload) and supports both legacy/new field names.
  - /api/public/app-settings served `no-store` to reflect Admin changes immediately.
  - getHomeHeroSettings supports both legacy and new CTA keys (cta_label/cta_href and cta_support_*).
...
## Update — Admin Banner CMS (2026-02-08)
- Added root-only Admin module: **/admin/banner** to manage Home hero banner content.
- Storage/upload: reuse existing signed upload API (`/api/admin/media/sign-upload`) with prefix `banners/`.
- New API: `/api/admin/banner` (root-only) to GET/PUT banner settings stored in `app_settings` key/value.
- Home reads banner config from `/api/public/app-settings` via `lib/app-settings.ts` keys:
  - `home.hero.image_url`, `home.hero.headline_top`, `home.hero.headline_mid`, `home.hero.headline_bottom`
  - `home.hero.cta_support_label`, `home.hero.search_placeholder`, `home.hero.diagnose_button_label`
- Writes audit log: `banner.update` on save.
...
## Update — v27 (2026-02-08)

Goal:
- A1 HERO + CTA: Admin Banner (Home Hero) save must reflect immediately on Home.

Issue:
- Admin banner showed "saved" but Home kept default texts/buttons.

Fix:
- Robust read for app_settings keys: tolerate duplicate rows (no UNIQUE(key)) by selecting all matches and choosing newest by updated_at/created_at/id.
- Robust write for app_settings keys: UPDATE by key first, INSERT if missing (prevents duplicates).
- Home keeps fetching hero config via /api/public/app-settings (no-store) so UI updates without redeploy.

Working set:
- lib/app-settings.ts (safeReadSetting)
- app/api/admin/banner/route.ts (upsertSetting)
