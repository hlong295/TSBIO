...
export function AdminShell({ title, children }: { title: string; children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Best-effort immediate token from sessionStorage to avoid blank admin while navigating.
    try {
      const cached = sessionStorage.getItem("tsbio_admin_access_token");
    ...
    async function syncToken(bestEffortRefresh: boolean) {
      ...
      try {
        ...
        // Cache token for fast restores during client-side navigation.
        try {
          if (accessToken) sessionStorage.setItem("tsbio_admin_access_token", accessToken);
          else sessionStorage.removeItem("tsbio_admin_access_token");
        } catch {
          // ignore
        }
