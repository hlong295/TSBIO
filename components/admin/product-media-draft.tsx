...
export function AdminProductMediaDraft({
  ...
  // Keep a tiny local debug state so any debug hooks never crash runtime.
  // (Previous builds referenced `setDbgState` without defining it.)
  const [dbgState, setDbgState] = React.useState<any>({});
  ...
  return (
    <div className="rounded-xl border p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {/* Hidden input (triggered from a real button click for maximum compatibility). */}
        <input
          ref={imageInputRef}
          type="file"
          accept="image/*"
        ...
        <button
          ...
          className={
            ...
            (disabled || totalImagesCount >= maxImages ? "cursor-not-allowed opacity-50" : "")
          }
          aria-disabled={disabled || totalImagesCount >= maxImages}
          onClick={() => {
            if (disabled || totalImagesCount >= maxImages) return;
            try {
              setDbgState((s: any) => ({ ...s, lastAction: "click_images", at: Date.now() }));
            } catch {
              // ignore
            }
            try {
              // Reset value so selecting the same file twice still triggers onChange.
              if (imageInputRef.current) (imageInputRef.current as any).value = "";
        ...
        <button
          ...
          className={
            "inline-flex items-center justify-center rounded-md border px-3 py-2 text-sm text-muted-foreground hover:bg-muted " +
            (disabled || hasVideo ? "cursor-not-allowed opacity-50" : "")
          }
          aria-disabled={disabled || hasVideo}
          onClick={() => {
            if (disabled || hasVideo) return;
            try {
              setDbgState((s: any) => ({ ...s, lastAction: "click_video", at: Date.now() }));
            } catch {
              // ignore
            }
            try {
              if (videoInputRef.current) (videoInputRef.current as any).value = "";
