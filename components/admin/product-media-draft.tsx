...
async function uploadTempSignedWithProgress(opts: {
  ...
  const res = await doFetch("/api/admin/media/tmp-sign-upload", {
    ...
    // Ensure cookie-based session works when Authorization is missing.
    // This helps on Pi Browser / webviews where the admin session is stored in cookies.
    credentials: "include",
  });
  ...
  // 2) Upload to Storage using the returned signed URL.
  // We use XHR to get upload progress and avoid some webview hangs observed with
  // supabase-js uploadToSignedUrl (stuck at 1%).
  const signedUrl = String(json.item.signedUrl || "");
  if (!signedUrl) throw new Error("SIGNED_UPLOAD_MISSING_URL");

  // Keep UI responsive even when progress events are not reported.
  let simulated = 1;
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      xhr.upload.onprogress = (e) => {
        const total = (e as any).total || file.size || 1;
...
function AdminMediaPickerInline(props: {
  ...
  const filtered = React.useMemo(() => {
    const kw = (typeof q==="string"?q.trim():String(q??"")).toLowerCase();
    if (!kw) return items;
...
export function AdminProductMediaDraft({
  ...
  async function handleChooseVideo(files: FileList | File[] | null) {
    ...
    try {
      ...
      // Upload video via signed URL (direct-to-storage). This is the same mechanism used by /admin/media
      // and avoids server-side buffering/timeouts for large files.
      ...
      if (thumbFile) {
        try {
          const thumbRes = await uploadTempSignedWithProgress({
            kind: "image",
            file: thumbFile,
            apiHeaders,
            onProgress: () => {},
