...
export function AdminProductMediaDraft({
  ...
  // NOTE: iOS/Safari can block programmatic file input clicks.
  // We rely on a real <label> click to open the picker reliably.
  ...
  return (
    <div className="rounded-xl border p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <label
          className={
            "relative inline-flex cursor-pointer items-center justify-center rounded-md border px-3 py-2 text-sm text-muted-foreground hover:bg-muted " +
            (disabled || totalImagesCount >= maxImages ? "cursor-not-allowed opacity-50" : "")
          }
          aria-disabled={disabled || totalImagesCount >= maxImages}
          onClick={(e) => {
            // Prevent opening picker when disabled.
            if (disabled || totalImagesCount >= maxImages) e.preventDefault();
          }}
        >
          <input
            type="file"
            accept="image/*"
            multiple
            // Keep input clickable (opacity 0) for iOS/Safari reliability.
        ...
        <label
          ...
          onClick={(e) => {
            if (disabled || hasVideo) e.preventDefault();
          }}
        >
          <input
            type="file"
