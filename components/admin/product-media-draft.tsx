...
type Props = {
  ...
  onChange: (next: MediaDraftState) => void;
  maxImages?: number;
  maxVideo?: number;
};
...
{
  const { kind, file, onProgress } = opts;
...
export function AdminProductMediaDraft({
  ...
  const { images, video, libraryImages, libraryVideoUrl } = value;
  ...
  async function removeImage(id: string) {
    ...
    if (target?.tmp_path) {
      await deleteTemp(target.tmp_path);
    }
  }
  ...
  return (
    <div className="rounded-xl border p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <label className="inline-flex cursor-pointer items-center justify-center rounded-md border px-3 py-2 text-sm text-muted-foreground hover:bg-muted">
          <input
            type="file"
            accept="image/*"
            multiple
