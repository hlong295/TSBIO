...
async function uploadTempWithProgress(opts: {
  ...
  return new Promise<{ tmp_path: string; url: string }>(async (resolve, reject) => {
    try {
      const { file, onProgress, apiHeaders } = opts;

      // Always try to ensure we have a Bearer token, because on some WebViews
      // the parent page may render before admin headers are hydrated.
      let headers: Record<string, string> = { ...(apiHeaders || {}) };
      ...
      xhr.open("POST", "/api/admin/media/tmp-upload", true);
      ...
      xhr.onload = () => {
        try {
          ...
          if (status < 200 || status >= 300) {
            // Try parse structured error first
            try {
              const j = JSON.parse(raw);
              reject(new Error(j?.error || `HTTP_${status}`));
            } catch {
              reject(new Error(raw || `HTTP_${status}`));
            }
            return;
          }
          const j = JSON.parse(raw);
          ...
          resolve({ tmp_path: j.tmp_path, url: j.url });
        } catch (e: any) {
          reject(new Error(e?.message || "UPLOAD_PARSE_ERROR"));
        }
      };
  ...
  return (
    <div className="rounded-xl border p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {/* Use native <label> → <input type=file> activation for maximum compatibility (Safari/WebView/Pi). */}
        <label
          className={
          ...
          aria-disabled={disabled || totalImagesCount >= maxImages}
        >
        ...
        <label
          ...
          aria-disabled={disabled || hasVideo}
        >
