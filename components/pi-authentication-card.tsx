...
export function PiAuthenticationCard({ onSuccess }: PiAuthenticationCardProps) {
  const { isAuthenticated, authMessage, piAccessToken, piUser, error, reinitialize } = usePiNetworkAuthentication()

  useEffect(() => {
    if (isAuthenticated && piAccessToken) {
      // Use best-effort real Pi identity.
      // - Pi SDK flow always provides username/uid
      // - App Studio iframe flow may only provide a token; in that case we fall back safely
      onSuccess(
        piAccessToken,
        piUser?.username ?? "Pi User",
        piUser?.uid ?? "pi-user-id"
      )
    }
  }, [isAuthenticated, piAccessToken, piUser, onSuccess])
